...
def help_message_handler(client, message):
    text = """
    **Префиксы**: [.] - [/] - [!] - [-]
**Команды**: https://telegra.ph/Aktualnye-komandy-04-15
...
@client.on_message(filters.command(`bulling`_command, prefixes=commands_prefix) & filters.me)
async def bulling_command_handler(client, message):
    try:
        ...
        if shabl in ["красивый-текст", "провокации", "дефолтные", "все", "лесенка", "тест"]:
            await message.delete()
            with open(f"shabloni_{shabl}.txt", encoding="UTF-8") as file:
                lines = file.readlines()

            print(f'{BLUE}[{AQUA}LOG{BLUE}]{WHITE} Буллинг был успешно запущен! Кол-во сообщений: {GOLD}{kol}{WHITE} | Тип шаблонов: {GOLD}{shabl}{WHITE}')
...
@client.on_message(filters.command(checker_command, prefixes=commands_prefix) & filters.me)
def check_command(client, message):
    chat_id = message.chat.id
    args = message.text.split()[1:]
    if len(args) < 2:
        client.send_message(chat_id, "Используй: /checker Тип запроса:[ping, http, tcp, dns, udp] Сайт/Айпи")
        return
    check_type = args[0]
    hostname = args[1]
    response = check_host(check_type, hostname)
    request_id = response.get("request_id")
    if request_id:
        client.send_message(chat_id, f"Чек-репорт: https://check-host.net/check-report/{request_id}")
    else:
        client.send_message(chat_id, "Ошибка при отправке запроса")

@Client.on_message(filters.command(killer_command, prefixes=commands_prefix) & filters.me)
async def killer_command(client, message):
    chat_id = message.chat.id
    await client.send_message(chat_id, "Абобус          💥 🔫", reply_to_message_id=message.message_id)

    # Определим начальные координаты 💥 и Абобуса
    explosion_x = 0
    bus_x = 9

    while explosion_x < bus_x:
        # Создаем строку анимации, смещая 💥 вправо
        animation = " " * explosion_x + "💥" + " " * (bus_x - explosion_x - 1) + " 🔫"
        await asyncio.sleep(1)
        await client.edit_message_text(chat_id, message.message_id, f"Абобус{animation}")

        # Увеличиваем координаты 💥 и Абобуса
        explosion_x += 1

    # Отправляем окончательное состояние анимации
    animation = " " * bus_x + "💥 🔫"
    await asyncio.sleep(1)
    await client.edit_message_text(chat_id, message.message_id, f"Абобус{animation}")
